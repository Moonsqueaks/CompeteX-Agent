...
| MVP 数据策略 | 用户提供真实脱敏 SKU 快照为主，公开页面采集为增强占位能力 |
| 报告输出 | 前端网页报告 + 后端 Markdown 导出 |
...
4. 通过竞争关系拨盘展示不同价格带、人群、场景下竞品关系的变化。
5. 通过 Agent Trace 页面展示多 Agent 协作过程，支撑比赛答辩。
...
6. 前端可以展示输入页、产品画像页、竞品战场页、报告页、Agent Trace 页。
7. 后端可以导出 Markdown 格式报告。
...
用户输入一个自动猫砂盆产品信息或选择预置 Demo 产品，系统启动分析任务。Orchestrator 调度 Collection Agent 读取 SKU 快照、评论快照和用户研究材料，Analysis Agent 建模产品画像和竞争关系，QA Agent 检查证据完整性并触发必要打回，Writer Agent 生成网页报告和 Markdown 报告。用户可以在前端查看竞争关系图、切换动态切片、查看证据和 QA 打回记录，并对关键结构化字段进行人工修正。
...
| Trace 展示 | 展示 Agent 运行、工具调用、Token、打回记录和前后差异 |
| 报告输出 | 网页报告展示，后端导出 Markdown |
...
| PDF/Word 导出 | Markdown 足够支撑答辩和留档 |
| 跨行业深度迁移 | 第一版聚焦自动猫砂盆 |
...
| 产品画像页 | 查看和修正目标产品结构化画像 | 基础信息、功能树、价格模型、人群场景、人工修正入口 |
| 竞品战场页 | 展示动态竞争关系，是主展示页 | 竞争关系图、切片拨盘、决策链、评分解释、证据卡片、QA 记录 |
| 报告页 | 展示最终分析报告 | 摘要、产品画像、竞品发现、动态切片、用户研究、建议、证据索引 |
| Agent Trace 页 | 展示多 Agent 协作过程 | DAG 状态、Agent 输入输出、工具调用、Token、QA 打回前后对比 |
...
| 字段 | 类型 | 必填 | 说明 |
|---|---|---:|---|
| target_product_name | 文本 | 是 | 目标产品名称 |
| target_product_url | 文本 | 否 | 商品链接，MVP 中作为记录字段 |
...
2. 当用户选择 `snapshot_plus_live` 时，提示公开页面采集可能失败，失败后自动降级到本地快照。
3. 点击启动后创建 `AnalysisTask`，进入任务运行状态。
...
| FeatureTree | 清洁能力、除臭能力、安全能力、智能能力、维护成本 |
| PricingModel | 标价、到手价、优惠、套餐、价格证据 |
| UserPersona | 目标人群、痛点、使用场景、决策因素 |
| Evidence | 商品页、评论、截图、调研材料 |
...
1. 前端提供导出按钮。
2. 后端接口生成 Markdown 文本。
3. Markdown 中保留 Claim、Evidence、置信度、访问时间和推断标识。
4. 不导出 API Key、完整系统 Prompt、未脱敏用户隐私信息。
...
| Agent Run | Agent 名称、开始时间、结束时间、状态、输入摘要、输出摘要 |
| Tool Call | 工具名称、参数摘要、成功/失败、耗时、错误信息 |
| Token Usage | prompt_tokens、completion_tokens、total_tokens |
| QA Review | 检查项、问题等级、打回目标、处理结果 |
| Diff View | 打回前后 Claim、Evidence 或评分变化 |

Prompt 可以折叠展示，并必须经过脱敏处理。
...
| Writer Agent | 生成网页报告数据和 Markdown 报告 | 通过 QA 的结构化产物 | ReportData、MarkdownReport |
...
| GET | `/tasks/{task_id}/battlefield` | 获取竞品战场数据 |
| GET | `/tasks/{task_id}/trace` | 获取 Agent Trace |
...
| JSON 文件 | SKU 快照、评论快照、Trace mock 数据 |
| 本地文件目录 | 截图、导出的 Markdown 报告 |
...
6. Prompt 展示到 Trace 页前需要脱敏，隐藏 API Key、用户隐私和过长原文。
...
| A6 | 竞品战场页可以切换价格带、人群、场景 |
| A7 | 每条核心 CompetitionEdge 展示评分解释和证据 |
| A8 | Agent Trace 页展示 DAG、工具调用、Token、打回记录 |
| A9 | 用户可以提交有限 HumanFeedback |
| A10 | 后端可以导出 Markdown 报告 |

### 18.2 演示验收

1. 使用本地快照完成稳定录屏。
2. 现场可以局部演示切片切换、证据查看、QA 打回记录和 Markdown 导出。
3. 答辩材料明确展示系统差异点：动态竞争关系评分、上下文切片、QA 真实打回、Agent Trace。
...
2. 抖音电商本地快照。
3. LangGraph 单任务链路。
4. 结构化报告和 Markdown 导出。
5. QA 打回和 Agent Trace。
...
| P0 | QA 真实打回案例 |
| P0 | 竞品战场页和 Agent Trace 页 |
| P1 | 产品画像页、报告页、Markdown 导出 |
...
1. 输入目标自动猫砂盆产品并启动任务。
2. 展示 LangGraph Agent Trace。
3. 展示产品画像和 Evidence。
4. 进入竞品战场页，切换价格带、人群、场景。
5. 展示某条 CompetitionEdge 的评分解释。
6. 展示 QA 打回案例和打回前后差异。
7. 展示网页报告和 Markdown 导出。
8. 总结系统价值和可扩展方向。
