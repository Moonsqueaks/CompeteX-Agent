...
CompeteX-Agent 是一个完整的竞品分析 Agent 系统：前端创建任务，后端启动 LangGraph 多 Agent 工作流，系统自动完成数据加载、竞品召回、竞争关系评分、证据质检、报告规划、LLM 生成、报告缓存、Markdown / Word 导出和过程追踪。

当前仓库内置两套可运行 Demo 场景：自动猫砂盆，以及互联网产品 / AI 助手。自动猫砂盆场景使用真实脱敏 SKU 快照、本地商品图片、评论摘要和用户研究文本验证完整链路；AI 助手场景使用豆包、Kimi、DeepSeek、千问、腾讯元宝等公开页快照和本地证据材料验证跨领域迁移能力。系统架构本身面向更广泛的消费品和互联网产品竞品分析任务，可继续扩展数据源、类目知识库和外部检索能力。
...
- 🧠 LLM 如何参与报告段落生成和质量修正。
- 🔒 报告为什么刷新后不变，Markdown / Word 为什么读取同一份缓存。
